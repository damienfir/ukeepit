...
namespace uKeepIt.MiniBurrow.Serialization
{
    public class BurrowObject
    {
        ...
        public static BurrowObject For(ObjectHeader objectHeader, byte[] data) { return For(objectHeader, new ArraySegment<byte>(data)); }

        public static BurrowObject For(ObjectHeader objectHeader, ArraySegment<byte> data)
        {
            // Allocate memory
            var headerLength = objectHeader.ByteLength();
            var bytes = new byte[headerLength + data.Count];

            // Serialize
            objectHeader.WriteToByteArray(bytes, 0);
            Array.Copy(data.Array, data.Offset, bytes, headerLength, data.Count);
            return new BurrowObject(new ArraySegment<byte>(bytes));
        }

        public static BurrowObject For(ObjectHeader objectHeader, ByteWriter data)
        {
            // Allocate memory
            var headerLength = objectHeader.ByteLength();
